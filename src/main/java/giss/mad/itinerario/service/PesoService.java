...
import giss.mad.marcocalidad.itinerariocalidad.repo.EjeHeredableRepository;
import giss.mad.marcocalidad.itinerariocalidad.repo.PesoRepository;
import giss.mad.marcocalidad.itinerariocalidad.model.auxpesos.PesoEjeIniciador;
...
public class PesoService {
    ...
    private Map<Integer, List<Integer>> pesosHeredablesForDeliveries = Map.of(1, List.of(7,15,16,20));//writables
    ...
    public Collection<PesoGraph> getAllByElement(Integer idElement, Boolean isDelivery) {
        Collection<Peso> c = this.pesoRepository.findAllByDeletedIsNullAndElementTypeIdAndForDelivery(idElement, isDelivery, Sort.by(Sort.Order.asc("id")));
        if (c == null) {
    ...
    public List<Integer> getAxisList() {
        ...
        String jsonString = restTemplate.exchange("/catalogo/ejes/getAll", HttpMethod.GET, entity, String.class).getBody();
    ...
    public void initializeDB() {
        ...
        for (Integer idElementType : this.pesosHeredablesForDeliveries.keySet()){
            List<Integer> ejesHeredablesForDeliveryList = this.pesosHeredablesForDeliveries.get(idElementType);
        ...
        Map<Map<Integer,Boolean>, Map<Integer, List<Weight>>> elementsOfCatalogo = new PesoEjeIniciador(restTemplate).getElementsOfCatalogo();
        int id = 1;
